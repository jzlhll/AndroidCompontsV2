...
class ScreenEffectView1 @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // 方法1: 使用路径操作实现精确的圆角渐变
        //drawWithPathOperation(canvas)
        
        // 或者使用方法2: 简单的渐变填充（注释掉上面的，取消注释下面的）
        drawSimpleGradient(canvas)
    }

    /**
     * 方法1: 使用路径操作实现精确的圆角渐变效果
     * 这种方法在圆角处效果更好
     */
    private fun drawWithPathOperation(canvas: Canvas) {
        // 保存画布状态
        canvas.save()
        ...
        // 使用PorterDuff模式清除内部圆角矩形区域
       // paint.xfermode = PorterDuffXfermode(PorterDuff.Mode.CLEAR)
        
        // 绘制内部圆角矩形路径（将被清除）
//        path.reset()
//        path.addRoundRect(innerRectF, cornerRadius, cornerRadius, Path.Direction.CW)
