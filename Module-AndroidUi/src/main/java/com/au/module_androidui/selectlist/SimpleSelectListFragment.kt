package com.au.module_androidui.selectlist
 ...
 * 选择一项。
 */
abstract class SimpleSelectListFragment<B: SimpleItem> : ViewToolbarFragment() {
    companion object {
        fun start(context:Context, fragment: Class<SimpleSelectListFragment<*>>) {
    ...
    override fun onUiCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        return NestedScrollView(inflater.context).also { scrollView->
            ...
            for (item in items) {
                val v = createItem(context, item, initCurrent)
                host.addView(
                    v,
